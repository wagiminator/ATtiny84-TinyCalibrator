...
void HVSP_writeEEPROM(uint16_t addr, uint8_t data) {
  ...
  if (target != TGT_T13) HVSP_sendInstr(addr>>8, 0x1C); // Instr2: address high byte
...
uint8_t waitButton(void) {
  TGT_VCC_ON();                     // target must be on to avoid interference
