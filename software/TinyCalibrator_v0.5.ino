...
#define HVSP_DELAY()      asm("nop")
#define HVSP_CLOCKOUT()   {HVSP_SCI_HIGH(); HVSP_DELAY(); HVSP_SCI_LOW();}
...
uint8_t HVSP_sendInstr(uint8_t SDI_BYTE, uint8_t SII_BYTE) {
  ...
  HVSP_SII_LOW();                     // SII = '0'
  HVSP_CLOCKOUT();                    // SCI HIGH, SCI LOW
  HVSP_DELAY();                       // 1 clock cycle delay
  HVSP_CLOCKOUT();                    // SCI HIGH, SCI LOW
...
void HVSP_readSignature(void) {
  ...
  HVSP_sendInstr(0x00, 0x0C);             // Instr2: select signature byte 0
...
const char DetectingStr[] PROGMEM =     "Detecting ...";
